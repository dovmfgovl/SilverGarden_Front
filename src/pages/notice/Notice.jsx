...
import {faList, faPenNib, faPenToSquare, faVolumeHigh } from '@fortawesome/free-solid-svg-icons'
  ...
  return (
    <div className={styles.noticeContainerLayout}>
      ...
      <div className={styles.innerContentLayout}>
        {noticePage === "전체공지"  && <NoticeList noticeList={noticeList} handlePage={handlePage} getList={getList}/>}
        {noticePage === "공지상세"  && <NoticeDetail noticeNo={noticeNo} handlePage={handlePage} fileList={fileList} handleFileList={handleFileList}/>}
        {noticePage === "공지작성"  && <NoticeWrite handlePage={handlePage}/>}
        {noticePage === "공지수정"  && <NoticeUpdate noticeNo={noticeNo} noticeList={noticeList} handlePage={handlePage} fileList={fileList}/>}
      </div>
    </div>
  )
}
