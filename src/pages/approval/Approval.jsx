...
const handleMenu = (menuTitle, d_no) =>{//사이드바 메뉴를 조작하는 함수
  setPage(menuTitle)
  if(d_no){//만약 d_no가 있다면(결재문서 상세조회라면)
    setDocNo(d_no)
  }
}
  ...
  return (
    <div className={styles.approvalWrap}>
      ...
      <div className={styles.approvalContentWrap}>
        {approvalPage === "결재대기함" && <ApprovalWaitList handleMenu={handleMenu} empData={empData}/>}
        {approvalPage === "결재진행함" && <ApprovalProgList handleMenu={handleMenu} empData={empData}/>}
        {approvalPage === "반려문서함" && <ApprovalDenyList handleMenu={handleMenu} empData={empData}/>}
