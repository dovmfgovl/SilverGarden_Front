...
import { messageCompleteDelete, messageFileDownload, messageStore } from '../../services/api/messageApi';

const MessageDeletedDetail = ({handleMenu, empData, messageDetail}) => {
  ...
  const handleCompleteDeleteBtn = async () =>{
    const response = await messageCompleteDelete({r_id:empData.e_no, me_no:messageDetail.me_no})
    console.log(response.data);
    if(response.data === "ok"){
      alert("쪽지가 삭제되었습니다.")
      handleMenu("받은쪽지함")
  ...
  return (
    <div className={styles.detailWrap}>
      <div className={styles.detailBtnGroup}>
        ...
        <Button className="mt-3" variant="outline-danger" onClick={handleCompleteDeleteBtn}>삭제</Button>{' '}
      </div>
