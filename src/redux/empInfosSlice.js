...
import { empEduUpdateDB, empListDB, empUpdateDB } from "../services/api/empInfoApi";
...
export const saveEmpEdu = createAsyncThunk(
  'empInfos/saveEmpEdu',
  async (updatedEdu) => {
    try {
      const response = await empEduUpdateDB(updatedEdu); // API 호출로 직원 정보 업데이트
      return response.data; // API 응답에서 데이터만 변환
    } catch (error) {
      // 에러 발생 시 처리
...
const empInfosSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(saveEmpEdu.fulfilled, (state, action) => {
        state.selectedEmployee = action.payload; // 선택된 직원 정보 업데이트
      }); 
  }
});
